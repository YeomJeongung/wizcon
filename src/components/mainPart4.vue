...
onMounted(() => {
  ...
  headers: {
    ...
    'Authorization' : 'eyJhbGciOiJIUzI1NiJ9.eyJ0aWQiOjM0OTMyMDc4NiwiYWFpIjoxMSwidWlkIjo1NzEyODc1NywiaWFkIjoiMjAyNC0wNC0xOVQwMToxNzozNS4wNTBaIiwicGVyIjoibWU6d3JpdGUiLCJhY3RpZCI6MjE3MzU2NDQsInJnbiI6ImFwc2UyIn0.DYngEzESkkGDmr1t_YXSvJhoFCKQC7DpeSXQs0XgUNw'
    },
    body: JSON.stringify({
      'query' : query
    })
...
const returnJsonDataFile = (data,target) => {
  let json = JSON.parse(data);
  if(json && json.files){
    return json.files[0][target];
  }else{
    return '';
  }
}
...
<template>
  <div class="box1 contents flx-col flx-l">
      ...
      <div class="w100 animation-duration-1000 animation-delay-1000 animation-ease-in-out"
      ...
      >
        <div class="webzins flx-row flx-wrap gap3">
          <template v-for="(item, idx) in items" :key="idx">
            <div class="item flx-row flx-v gap2" style="width: calc(50% - 1.5rem);" v-for="(itemIn, idxs) in item" :key="idxs">
              <div class="img">
                <template v-if="returnJsonDataFile(itemIn.column_values[4].value,'isImage') == 'true'">                
                  <Image :src="`${returnJsonDataFile2(itemIn.assets,'public_url')}`" alt="Image" preview />
                </template>
                <template v-else>
                  <div class="noimg">
                    <Image src="/src/assets/images/logo.svg" alt="Image" />
                  </div>
                </template>
              </div>
              <div class="info flx-col h100">
                <span class="red mb1 txtline t_1">{{ returnJsonData(itemIn.column_values[2].value,'date') }}</span>
                <template v-if="itemIn.column_values[3].value == null">
                  <h3 :class="['font1-5 mb1 txtline t_2']">
                    {{ itemIn.name }}
                  </h3>
                </template>
                <h3 v-else :class="['font1-5 mb1 txtline t_2', JSON.parse(itemIn.column_values[3].value)?.text != '' ? 'hand':'']" @click="detailDialog(itemIn.column_values[3].value,itemIn.name)"
                v-tooltip.top="{
                ...
                >{{ itemIn.name }}</h3>
                <div class="color999  txtline t_3">{{ itemIn.column_values[1].value.replace(/"/g, '') }}</div>
              </div>
            </div>
          </template>
        </div>  
      ...
      <Dialog v-model:visible="dialogVisible" maximizable modal :header="detailTitle" :style="{ width: '50rem' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
